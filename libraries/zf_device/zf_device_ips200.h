...
#endif
// 如果使用的是单排排针的两寸屏幕 SPI 驱动控制引脚 可以修改
#define IPS200_RST_PIN_SPI              (P02_8)                                 // 单排针(SPI)液晶复位引脚定义（按你接线）
#define IPS200_DC_PIN_SPI               (P02_5)                                 // 单排针(SPI)液晶命令位引脚定义（按你接线）
#define IPS200_CS_PIN_SPI               (P02_4)                                 // 单排针(SPI)液晶片选引脚定义（需连接或拉地）
#define IPS200_BLk_PIN_SPI              (P15_4)                                 // 单排针(SPI)液晶背光引脚定义（可接 3V3 或此脚）
#define IPS200_USE_BLK_PIN              (1)                                     // 置 1 使用背光控制引脚（初始化时会拉高 BLK）

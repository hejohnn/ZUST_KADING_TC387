 ...
 *
 * 硬件 (DRV8701E, PWM + DIR 模式):
 *     左后轮:  PWM = P33.12 (ATOM2_CH4)         DIR = P33.13 (GPIO)
 *     右后轮:  PWM = P33.5  (ATOM2_CH1)         DIR = P33.11 (GPIO)
 *     方向引脚: 高电平=正转, 低电平=反转
 *     速度引脚: PWM 占空比控制转速, 0 = 停车
 *
 * 调用顺序:
 *     Pedal_Init();
 *     Motor_Init();
...
extern int16 menu_motor_left_duty;     /* 左轮当前占空比 (带符号, 正=前进, 负=后退) */
extern int16 menu_motor_right_duty;    /* 右轮当前占空比 (带符号) */
